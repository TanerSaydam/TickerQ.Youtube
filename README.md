# 🚀 TickerQ YouTube Eğitim Reposu

Bu repo, YouTube'da yayınladığım **TickerQ: Background Job** eğitim videosunda geliştirilen örnek projeyi içermektedir.

## 📚 İçerik

Bu eğitimde aşağıdaki konular anlatılmaktadır:

- TickerQ kurulumu
- Dashboard kullanımı
- Manual Job oluşturma
- Time Ticker
- Cron Ticker
- Background Job oluşturma
- `MapTicker` kullanımı
- `TickerFunction` Attribute kullanımı
- Dependency Injection
- EF Core entegrasyonu
- SQL Server üzerinde Job Persistence
- MaxConcurrency kullanımı

---

## 🎥 YouTube Videosu

👉 https://www.youtube.com/watch?v=OCCE_iS0gLk

---

## 📖 Resmi Doküman

https://tickerq.net/docs

---

## 💻 GitHub Projesi

https://github.com/arcenox-co/TickerQ

---

## ⭐ Destek Olmak İçin

Eğer repo faydalı olduysa;

- ⭐ GitHub reposuna Star vermeyi
- 👍 YouTube videosunu beğenmeyi
- 🔔 Kanala abone olmayı

unutmayın.

İyi çalışmalar. 😊
